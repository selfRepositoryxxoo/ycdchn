...
var $navScroll = new IScroll("#nav", {
    scrollbars: true,//这个是显示滚动条  不要忘了滚动条时通过定位的 ，所以在css必须有nav上必须有宽度和 position：relative；
    mouseWheel: true,//是否有滚轮驱动 true是滚轮是可以得
    bounce: false//是否有缓冲的状态；
});
...
$(function () {
    $.ajax({
        ...
        type: "get",
        dataType: "jsonp",
        jsonpCallback: "getHotMatchList",//这是浏览器告诉服务器你返回的匿名函数的名字；
...
$calendarCallbackList.add(function (today, data) {
    ...
    var endIn = strIn + 6;//在我们还没有点击默认显示的7天；
...
$(".calendar").on("click", function (ev) {
    ...
    if ($tarLink.length > 0) {
        ...
        $calListUL.finish().animate({left: curL}, 300, function () {
            //这个finish是为了快速点击的上个动画没有完成，下个动画就开始，必须等你完成，
            //stop好像是之间关闭你的动画；让他成为110的整数倍就不会有这个问题；
...
//这是切换热门还是 普通的通过那个columnId 每个区域的columnId是不一样的，只能靠这个
$("#nav li").on("click", function () {
    ...
    if (index > 0) {
        ...
        getCalendarList(columnId);//执行这个方法，里面有发布模式，
    }
